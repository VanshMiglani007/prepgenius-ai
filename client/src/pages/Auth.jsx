...
const Auth = () => {
  ...
  useEffect(() => {
    const initGoogle = () => {
      if (window.google?.accounts?.id && GOOGLE_CLIENT_ID) {
        ...
        const els = document.getElementsByClassName('googleSignInDiv');
        for(let i=0; i<els.length; i++) {
          window.google.accounts.id.renderButton(
            els[i],
            { theme: "outline", size: "large", width: 330, shape: "rectangular" }
          );
        }
      }
    };
  ...
  const GoogleButton = () => {
    ...
    return (
      <div className="flex justify-center w-full mt-3 h-[40px] items-center overflow-hidden rounded-lg relative bg-white">
        <div className="googleSignInDiv absolute top-0 -left-[1px]"></div>
      </div>
    );
  };

  return (
    <div className="flex flex-col justify-center items-center min-h-screen p-5 text-white font-sans overflow-hidden relative" style={{ backgroundColor: 'rgb(var(--color-bg))' }}>
